...
def display_all_states(state_tracker: list):
    """ Display all states in the state tracker """
    print("Displaying all states:")
    for i, state in enumerate(state_tracker):
        display_cube(state, title=f"State at Iteration {i+1}")
        print()
...
def visualize_experiment(results, algo_name="Algorithm"):
    ...
    if "state_tracker" in results:
        display_all_states(results["state_tracker"])
    # Print the results
